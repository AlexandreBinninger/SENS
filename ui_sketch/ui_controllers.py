...
from ui_sketch import ui_utils, inference_processing, gaussian_status
import options
...
class GmmMeshStage:
    ...
    def remove_all(self):
        ...
        self.addresses_dict = {}
        self.gmm = []
        self.addresses_dict: Dict[str, int] = {}
    ...
    def split_mesh_by_gmm(self, mesh, clusters: TN = None) -> Dict[int, T]:
        faces_split = {}
        if clusters is None:
            ...
            faces_split_ = gm_utils.split_mesh_by_clusters(mesh,  clusters, len(self.gmm))
        counter = 0
    ...
    def save(self, root: str, filter_faces: Callable[[gaussian_status.GaussianStatus], bool] = filter_by_inclusion):
        if self.faces is not None:
            ...
            faces_inds = map(lambda x:
                             torch.ones(x[1].shape[0], dtype=torch.int64) + self.gmm_id
    ...
    def replace_mesh(self, mesh: Optional[V_Mesh]):
        self.raw_gmm = files_utils.load_gmm("./assets/tmp/tmp_gmm", as_np=True)[:-1]
        self.gmm = self.add_gmm()
    ...
    def __init__(self, opt: options.Options, shape_id: int, render: ui_utils.CanvasRender, render_number: int,
        ...
        self.gmm: List[gaussian_status.GaussianStatus] = []
...
class GmmStatuses:
    ...
    def __init__(self, opt: options.Options, shape_ids: List[int], render, view_styles: List[ui_utils.ViewStyle]):
        self.gmms = [GmmMeshStage(opt, shape_id, render, i, view_style) for i, (shape_id, view_style) in
                     enumerate(zip(shape_ids, view_styles))]
...
class MeshGmmStatuses(GmmStatuses):
    ...
    def save_light(self, root):
        save_dict = {gmm.shape_id: [gaussian.gaussian_id[1] for gaussian in gmm.gmm if gaussian.included]
    ...
    def __init__(self, opt: options.Options, shape_ids: Union[Tuple[int, ...], List[int]], render, view_styles: List[ui_utils.ViewStyle],
                 with_model: bool):
        super(MeshGmmStatuses, self).__init__(opt, shape_ids, render, view_styles)
        if with_model:
