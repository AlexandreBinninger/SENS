...
def igl_prepare(*dtypes):
    ...
    def decoder(func):
        ...
        if len(dtypes) == 0:
            ...
            to_torch = to_torch_single
        else:
            to_torch = to_torch_multi
        return wrapper

    def to_torch_single(result, device):
...
@igl_prepare()
def get_fast_inside_outside(mesh: T_Mesh, points: ARRAY) -> ARRAY:
    batch_size = 1000000
    labels = []
    num_batch = points.shape[0] // batch_size + 1
    for i in range(points.shape[0] // batch_size + 1):
...
def remove_duplicate_faces(mesh: T_Mesh, epsilon=1e-5) -> T_Mesh:
    faces_old = mesh[1].sort(1)[0]
    faces_old = [tuple(item) for item in faces_old.tolist()]
    face_stash = set()
    select = []
    for i, face in enumerate(faces_old):
        if face not in face_stash:
            face_stash.add(face)
            select.append(i)
    faces = mesh[1][torch.tensor(select, dtype=torch.int64)]
    return mesh[0], faces
