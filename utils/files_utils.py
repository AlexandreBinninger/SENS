...
def save_image(image: ARRAY, path: str, mode=None):
...
def load_ply(path: str, get_normals=False):
    plydata = plyfile.PlyData.read(path)
    vertices = plydata.elements[0].data
    vs = [[float(item[0]), float(item[1]), float(item[2])] for item in vertices]
    vs = torch.tensor(vs)
    if get_normals:
        if len(plydata.elements[0].data[0]) > 3 :
            normals = [[float(item[3]), float(item[4]), float(item[5])] for item in vertices]
